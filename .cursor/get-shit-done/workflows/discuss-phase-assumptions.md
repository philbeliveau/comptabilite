...
<answer_validation>
**IMPORTANT: Answer validation** — After every conversational prompting call, check if the response
...
When text mode is active, do not use conversational prompting at all. Present every question as a
...
AGENT_SKILLS_ANALYZER=$(node "/Users/philippebeliveau/Desktop/Notebook/comptabilite/.cursor/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-assumptions-analyzer 2>/dev/null)
```
...
**If matches found:** Present matched todos, use conversational prompting (multiSelect) to fold relevant ones into scope.
...
Use conversational prompting (multiSelect):
- header: "Corrections"
...
```bash
node "/Users/philippebeliveau/Desktop/Notebook/comptabilite/.cursor/get-shit-done/bin/gsd-tools.cjs" commit "docs(${padded_phase}): capture phase context (assumptions mode)" --files "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
