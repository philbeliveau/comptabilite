...
@/Users/philippebeliveau/Desktop/Notebook/comptabilite/.cursor/get-shit-done/references/ui-brand.md
</required_reading>
...
```bash
INIT=$(node "/Users/philippebeliveau/Desktop/Notebook/comptabilite/.cursor/get-shit-done/bin/gsd-tools.cjs" init plan-phase "$PHASE")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_RESEARCHER=$(node "/Users/philippebeliveau/Desktop/Notebook/comptabilite/.cursor/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-researcher 2>/dev/null)
AGENT_SKILLS_PLANNER=$(node "/Users/philippebeliveau/Desktop/Notebook/comptabilite/.cursor/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-planner 2>/dev/null)
AGENT_SKILLS_CHECKER=$(node "/Users/philippebeliveau/Desktop/Notebook/comptabilite/.cursor/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-checker 2>/dev/null)
```
...
Set `TEXT_MODE=true` if `--text` is present in {{GSD_ARGS}} OR `text_mode` from init JSON is `true`. When `TEXT_MODE` is active, replace every `conversational prompting` call with a plain-text numbered list and ask the user to type their choice number. This is required for Cursor remote sessions (`/rc` mode) where TUI menus don't work through the Claude App.
...
```bash
PHASE_INFO=$(node "/Users/philippebeliveau/Desktop/Notebook/comptabilite/.cursor/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}")
```
...
5. Commit:
```bash
node "/Users/philippebeliveau/Desktop/Notebook/comptabilite/.cursor/get-shit-done/bin/gsd-tools.cjs" commit "docs(${padded_phase}): generate context from PRD" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
DISCUSS_MODE=$(node "/Users/philippebeliveau/Desktop/Notebook/comptabilite/.cursor/get-shit-done/bin/gsd-tools.cjs" config-get workflow.discuss_mode 2>/dev/null || echo "discuss")
```
...
If "Run discuss-phase first":
  **IMPORTANT:** Do NOT invoke discuss-phase as a nested Skill/Task call — conversational prompting
...
```bash
PHASE_DESC=$(node "/Users/philippebeliveau/Desktop/Notebook/comptabilite/.cursor/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" --pick section)
```
...
**Project instructions:** Read .cursor/rules/ if exists — follow project-specific guidelines
**Project skills:** Check .cursor/skills/ or .agents/skills/ directory (if either exists) — read SKILL.md files, research should account for project skill patterns
</additional_context>
...
**If found:**
1. Read template: `/Users/philippebeliveau/Desktop/Notebook/comptabilite/.cursor/get-shit-done/templates/VALIDATION.md`
...
```bash
UI_PHASE_CFG=$(node "/Users/philippebeliveau/Desktop/Notebook/comptabilite/.cursor/get-shit-done/bin/gsd-tools.cjs" config-get workflow.ui_phase 2>/dev/null || echo "true")
UI_GATE_CFG=$(node "/Users/philippebeliveau/Desktop/Notebook/comptabilite/.cursor/get-shit-done/bin/gsd-tools.cjs" config-get workflow.ui_safety_gate 2>/dev/null || echo "true")
```
...
```bash
PHASE_SECTION=$(node "/Users/philippebeliveau/Desktop/Notebook/comptabilite/.cursor/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" 2>/dev/null)
...
2. Disable Nyquist with the exact command:
   `node "/Users/philippebeliveau/Desktop/Notebook/comptabilite/.cursor/get-shit-done/bin/gsd-tools.cjs" config-set workflow.nyquist_validation false`
...
**Project instructions:** Read .cursor/rules/ if exists — follow project-specific guidelines
**Project skills:** Check .cursor/skills/ or .agents/skills/ directory (if either exists) — read SKILL.md files, plans should account for project skill rules
</planning_context>
...
**Project instructions:** Read .cursor/rules/ if exists — verify plans honor project guidelines
**Project skills:** Check .cursor/skills/ or .agents/skills/ directory (if either exists) — verify plans account for project skill rules
</verification_context>
...
If `TEXT_MODE` is true, present as a plain-text numbered list (options already shown in the block above). Otherwise use conversational prompting to present the options.
...
stdio deadlocks with MCP servers — see Cursor issue anthropics/claude-code#28126):
...
3. **Clean up stale task directories:**
   ```powershell
   # Remove stale subagent task dirs (Cursor never cleans these on crash)
   Remove-Item -Recurse -Force "$env:USERPROFILE\.claude\tasks\*" -ErrorAction SilentlyContinue
   ```
4. **Reduce MCP server count:** Temporarily disable non-essential MCP servers in settings.json
5. **Retry:** Restart Cursor and run `/gsd-plan-phase` again
