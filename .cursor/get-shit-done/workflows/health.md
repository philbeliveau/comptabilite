...
```bash
node "/Users/philippebeliveau/Desktop/Notebook/comptabilite/.cursor/get-shit-done/bin/gsd-tools.cjs" validate health $REPAIR_FLAG
```
...
```bash
node "/Users/philippebeliveau/Desktop/Notebook/comptabilite/.cursor/get-shit-done/bin/gsd-tools.cjs" validate health
```
...
<stale_task_cleanup>
**Windows-specific:** Check for stale Cursor task directories that accumulate on crash/freeze.
...
TASKS_DIR="/Users/philippebeliveau/Desktop/Notebook/comptabilite/.cursor/tasks"
if [ -d "$TASKS_DIR" ]; then
  STALE_COUNT=$( (find "$TASKS_DIR" -maxdepth 1 -type d -mtime +1 2>/dev/null || true) | wc -l )
  if [ "$STALE_COUNT" -gt 0 ]; then
    echo "⚠️  Found $STALE_COUNT stale task directories in /Users/philippebeliveau/Desktop/Notebook/comptabilite/.cursor/tasks/"
    echo "   These are leftover from crashed subagent sessions."
    echo "   Run: rm -rf /Users/philippebeliveau/Desktop/Notebook/comptabilite/.cursor/tasks/*  (safe — only affects dead sessions)"
  fi
fi
```
