...
<answer_validation>
**IMPORTANT: Answer validation** — After every conversational prompting call, check if the response is empty or whitespace-only. If so:
...
When text mode is active, **do not use conversational prompting at all**. Instead, present every
question as a plain-text numbered list and ask the user to type their choice number.
This is required for Cursor remote sessions (`/rc` mode) where the Claude App
...
```bash
INIT=$(node "/Users/philippebeliveau/Desktop/Notebook/comptabilite/.cursor/get-shit-done/bin/gsd-tools.cjs" init phase-op "${PHASE}")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_ADVISOR=$(node "/Users/philippebeliveau/Desktop/Notebook/comptabilite/.cursor/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-advisor 2>/dev/null)
```
...
- In `discuss_areas`: for each discussion question, choose the recommended option (first option, or the one marked "recommended") without using conversational prompting
...
```bash
TODO_MATCHES=$(node "/Users/philippebeliveau/Desktop/Notebook/comptabilite/.cursor/get-shit-done/bin/gsd-tools.cjs" todo match-phase "${PHASE_NUMBER}")
```
...
Use conversational prompting (multiSelect) asking which todos to fold into this phase's scope:
...
3. Resolve model for advisor agents:
   ```bash
   ADVISOR_MODEL=$(node "/Users/philippebeliveau/Desktop/Notebook/comptabilite/.cursor/get-shit-done/bin/gsd-tools.cjs" resolve-model gsd-advisor-researcher --raw)
   ```
...
**If `--auto`:** Auto-select ALL gray areas. Log: `[auto] Selected all gray areas: [list area names].` Skip the conversational prompting below and continue directly to discuss_areas with all areas selected.
...
2. For EACH user-selected gray area, spawn a Task() in parallel:
   ...
   Task(
     prompt="First, read @/Users/philippebeliveau/Desktop/Notebook/comptabilite/.cursor/agents/gsd-advisor-researcher.md for your role and instructions.
...
2. **Use conversational prompting:**
   - header: "{area_name}"
   - question: "Which approach for {area_name}?"
   - options: Extract from the table's Option column (conversational prompting adds "Other" automatically)
...
4. **After recording pick, Claude decides whether follow-up questions are needed:**
   - If the pick has ambiguity that would affect downstream planning → ask 1-2 targeted follow-up questions using conversational prompting
...
- When active, replace ALL `conversational prompting` calls with plain-text numbered lists
- User types a number to select, or types free text for "Other"
- This is required for Cursor remote sessions (`/rc` mode) where TUI menus
...
**Auto mode (`--auto`):** For each area, Claude selects the recommended option (first option, or the one explicitly marked "recommended") for every question without using conversational prompting. Log each auto-selected choice:
```
...
2. **Ask questions using the selected pacing:**
   ...
   **Default (no `--batch`): Ask 4 questions using conversational prompting**
   ...
   **Batch mode (`--batch`): Ask 2-5 numbered questions in one plain-text turn**
   ...
   - When options are helpful, include short inline choices per question rather than a separate conversational prompting for every item
...
```bash
node "/Users/philippebeliveau/Desktop/Notebook/comptabilite/.cursor/get-shit-done/bin/gsd-tools.cjs" commit "docs(${padded_phase}): capture phase context" --files "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
...
```bash
node "/Users/philippebeliveau/Desktop/Notebook/comptabilite/.cursor/get-shit-done/bin/gsd-tools.cjs" state record-session \
...
```bash
node "/Users/philippebeliveau/Desktop/Notebook/comptabilite/.cursor/get-shit-done/bin/gsd-tools.cjs" commit "docs(state): record phase ${PHASE} context session" --files .planning/STATE.md
```
</step>
...
```bash
node "/Users/philippebeliveau/Desktop/Notebook/comptabilite/.cursor/get-shit-done/bin/gsd-tools.cjs" config-set workflow._auto_chain_active true
```
