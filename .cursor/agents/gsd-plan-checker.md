...
**Question:** Do plans respect project-specific conventions, constraints, and requirements from .cursor/rules/?

**Process:**
1. Read `.cursor/rules/` in the working directory (already loaded in `<project_context>`)
...
4. Flag plans that introduce patterns .cursor/rules/ explicitly forbids
5. Flag plans that skip steps .cursor/rules/ explicitly requires (e.g., required linting, specific test frameworks, commit conventions)

**Red flags:**
- Plan uses a library/pattern .cursor/rules/ explicitly forbids
- Plan skips a required step (e.g., .cursor/rules/ says "always run X before Y" but plan omits X)
- Plan introduces code style that contradicts .cursor/rules/ conventions
- Plan creates files in locations that violate .cursor/rules/'s architectural constraints
- Plan ignores security requirements documented in .cursor/rules/

**Skip condition:** If no `.cursor/rules/` exists in the working directory, output: "Dimension 10: SKIPPED (no .cursor/rules/ found)" and move on.
...
issue:
  ...
  severity: blocker
  description: "Plan uses Jest for testing but .cursor/rules/ requires Vitest"
  plan: "01"
  task: 1
  claude_md_rule: "Testing: Always use Vitest, never Jest"
  plan_action: "Install Jest and create test suite..."
  fix_hint: "Replace Jest with Vitest per project .cursor/rules/"
```
...
issue:
  dimension: claude_md_compliance
  severity: warning
  description: "Plan does not include lint step required by .cursor/rules/"
  plan: "02"
...
```bash
INIT=$(node "/Users/philippebeliveau/Desktop/Notebook/comptabilite/.cursor/get-shit-done/bin/gsd-tools.cjs" init phase-op "${PHASE_ARG}")
...
node "/Users/philippebeliveau/Desktop/Notebook/comptabilite/.cursor/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "$phase_number"
...
for plan in "$PHASE_DIR"/*-PLAN.md; do
  echo "=== $plan ==="
  PLAN_STRUCTURE=$(node "/Users/philippebeliveau/Desktop/Notebook/comptabilite/.cursor/get-shit-done/bin/gsd-tools.cjs" verify plan-structure "$plan")
  echo "$PLAN_STRUCTURE"
done
```
...
```bash
MUST_HAVES=$(node "/Users/philippebeliveau/Desktop/Notebook/comptabilite/.cursor/get-shit-done/bin/gsd-tools.cjs" frontmatter get "$PLAN_PATH" --field must_haves)
```
...
```bash
PLAN_STRUCTURE=$(node "/Users/philippebeliveau/Desktop/Notebook/comptabilite/.cursor/get-shit-done/bin/gsd-tools.cjs" verify plan-structure "$PLAN_PATH")
```
...
- [ ] .cursor/rules/ compliance checked (plans respect project conventions)
