...
<task type="auto">
  <name>Task 2: Create CCA, shareholder loan, and CPA export stub extensions with tests</name>
  <files>
    src/compteqc/fava_ext/dpa_qc/__init__.py
    src/compteqc/fava_ext/dpa_qc/templates/DpaQCExtension.html
    src/compteqc/fava_ext/pret_actionnaire/__init__.py
  ...
  <action>
    ...
    - Class `PretActionnaireExtension(FavaExtensionBase)`:
      ...
      - `after_load_file()`: call `obtenir_etat_pret()` from `compteqc.quebec.pret_actionnaire` to get loan status from ledger entries.
  ...
  <done>
    All 4 Quebec extensions (payroll, GST/QST, CCA, shareholder loan) plus CPA export stub created with Python classes and Jinja2 templates. All registered in main.beancount. Shareholder loan includes s.15(2) countdown with color-coded alert levels. CCA shows all 5 classes (8, 10, 12, 50, 54). Tests pass for importability, report titles, template existence, and alert logic.
  </done>
</task>
