...
must_haves:
  truths:
    ...
    - "Transactions with confiance >= 0.95, revue_obligatoire=False, and montant <= $2,000 are auto-approved and bypass the pending queue"
    - "lancer_paie confirmation response includes raison field distinguishing nouveau_montant, gros_montant, or nouveau_et_gros_montant"
...
<task type="auto">
  ...
  <action>
    ...
    Update `services.py` to add `trouver_pending_par_id(entries: list, id: str) -> int | None`:
    - Call `lire_pending()` to get the pending transaction list.
    - For each entry, build composite key: `f"{entry.date}|{entry.payee}|{(entry.narration or '')[:20]}"`.
    - Return the 0-based index matching the given id string, or None if not found.
    - `approuver_lot` calls this function to convert each string id to an int index before calling `approuver_transactions(chemin_pending, chemin_main, indices)`.
    - `rejeter` likewise calls this to convert its single string id to an int index before calling `rejeter_transactions(chemin_pending, indices)`.

    Auto-approve logic: In `proposer_categorie`, after running the pipeline, if `confiance >= 0.95` AND `revue_obligatoire is False` AND `abs(montant) <= 2000`, call `approuver_transactions()` directly to bypass the pending queue. Return `{auto_approuve: True, ...}` in the response. Otherwise, write to pending as normal.
...
<task type="auto">
  ...
  <action>
    ...
    2. `lancer_paie(salaire_brut: str, nb_periodes: int = 26, offset_pret: str | None = None, confirmer: bool = False, ctx)` -- compute AND write payroll to ledger.
       - Check read_only flag.
       - Single unified confirmation guard. Determine `raison` before proceeding:
         - If salaire_brut differs from previous payroll AND gross > $2,000: raison = "nouveau_et_gros_montant"
         - If salaire_brut differs from previous payroll (but <= $2,000): raison = "nouveau_montant"
         - If gross > $2,000 (but same as previous): raison = "gros_montant"
         - Otherwise: raison = None (no confirmation needed)
       - If raison is not None and confirmer is False, return {status: "confirmation_requise", raison: "<one of the above>", message: "<French explanation of why confirmation is needed>", calcul: <dry-run results>}. This lets Claude distinguish which guard triggered.
       - If confirmer is True OR raison is None, proceed.
    ...
    - Test lancer_paie unified confirmation guard: raison="gros_montant" when same amount but >$2000, raison="nouveau_montant" when different amount <=2000, raison="nouveau_et_gros_montant" when both
    - Test lancer_paie in read_only mode returns French error
    - Test proposer_categorie auto-approve: confiance=0.98, revue_obligatoire=False, montant=500 => auto_approuve=True in response
    - Test proposer_categorie NO auto-approve: confiance=0.98 but montant=3000 => goes to pending
