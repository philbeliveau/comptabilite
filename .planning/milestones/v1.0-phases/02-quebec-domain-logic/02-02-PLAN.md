...
Function `generer_transaction_paie(date_paie: datetime.date, resultat: ResultatPaie, salary_offset: Decimal | None = None) -> data.Transaction`:
...
- If `salary_offset` is not None and > 0:
  - Add posting: Dr `Passifs:Pret-Actionnaire` = salary_offset (reduces shareholder loan balance)
  - Adjust Cr `Actifs:Banque:RBC:Cheques` to `-(net - salary_offset)` instead of `-net`
  - Add metadata: `salary_offset: str(salary_offset)`
  - Validate: salary_offset <= net (raise ValueError if not)
  - The `MouvementPret.type='salaire_offset'` from plan 02-04 will recognize these postings via the `Passifs:Pret-Actionnaire` account and the `salary_offset` metadata.
- Tags: `frozenset({"paie"})`
...
- `--salary-offset: Decimal` (optional, default None — when provided, apply this amount from net pay against shareholder loan. Reduces net pay by offset amount, adds posting: Dr `Passifs:Pret-Actionnaire` / Cr reduce `Actifs:Banque:RBC:Cheques` by offset less. Net bank deposit = net - offset.)
...
3. If `--salary-offset` provided: validate offset <= net pay, pass offset to `generer_transaction_paie()`. The journal generator must add an extra posting: Dr `Passifs:Pret-Actionnaire` for the offset amount, and reduce the Cr `Actifs:Banque:RBC:Cheques` posting by offset (bank deposit = net - offset). Display the offset line in the Rich breakdown table under a "Compensation pret actionnaire" section showing: offset amount, remaining net deposited.
4. If not dry-run: generate Beancount transaction via `generer_transaction_paie()`, write to monthly file using `ledger.fichiers` utilities, validate with bean-check, auto-commit
5. If dry-run: print "[Mode simulation — aucune ecriture au ledger]"
...
- Test salary offset: `generer_transaction_paie(date, resultat, salary_offset=Decimal("1000"))` → verify transaction has a Dr `Passifs:Pret-Actionnaire` posting for $1,000, and Cr `Actifs:Banque:RBC:Cheques` is reduced by $1,000 (net - 1000). Verify transaction still balances to zero.
- Test salary offset exceeds net: `salary_offset > net` raises ValueError
  </action>
  <verify>
