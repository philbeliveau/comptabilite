...
<task type="auto">
  ...
  <action>
    ...
    PREREQUISITE: Update `src/compteqc/quebec/pret_actionnaire/__init__.py` to export `obtenir_etat_pret`:
    - Add `from compteqc.quebec.pret_actionnaire.suivi import obtenir_etat_pret` to the imports.
    - Add `"obtenir_etat_pret"` to the `__all__` list.
    - Note: `obtenir_etat_pret(entries: list, fin_exercice: datetime.date) -> EtatPret` requires a `fin_exercice` argument.
    ...
    3. `etat_pret_actionnaire(ctx)` -- shareholder loan status. Call `obtenir_etat_pret(entries, fin_exercice)` from `compteqc.quebec.pret_actionnaire`. Use current fiscal year-end date (derive from entries or default to Dec 31 of current year). Return {solde_net, direction (corp_doit/actionnaire_doit), avances: [...], delai_inclusion_s152, jours_restants, alerte}.
